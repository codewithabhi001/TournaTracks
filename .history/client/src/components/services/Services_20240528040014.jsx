...
function ServiceCard({ title, description, color }) {
  return (
    <div
      className={`relative rounded-lg p-8 overflow-hidden group border-2 border-${color}-500 shadow-[0_9px_0_rgb(0,0,0)] transition-all ease-out hover:shadow-[0_4px_0px_rgb(0,0,0)] hover:translate-y-1`}
      style={{
    ...
    >
      ...
      <h3 className="relative text-2xl font-bold text-white mb-4 group-hover:text-${color}-200 transition duration-300">
        {title}
      </h3>
      <p className="relative text-lg text-white group-hover:text-${color}-200 transition duration-300">
        {description}
      </p>
    </div>
  );
