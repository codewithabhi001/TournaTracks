...
function Services() {
  return (
    <div className="container mx-auto mt-10 px-8 mb-28 light2 ">
...
function ServiceCard({ title, description, color, borderColor, shadowColor }) {
  return (
    <div
      className={` light2 relative rounded-lg p-8 overflow-hidden group mb-10 transition-all ease-out hover:translate-y-1 ${borderColor}`}
      style={{
    ...
    >
      <div
        className={`absolute inset-0 bg-opacity-50 bg-gray-900 blur-sm group-hover:blur-md transition duration-300 ${shadowColor} hover:shadow-lg`}
      ></div>
      <div
        className={`absolute inset-0 bg-${color}-500 opacity-50 transition duration-300 group-hover:opacity-30`}
      ></div>
